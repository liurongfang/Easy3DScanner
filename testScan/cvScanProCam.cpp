...
int slScan(/*CvCapture* capture,*/ 
	...
	// 创建窗口显示捕捉到的图像.
	IplImage* cam_frame  = NULL/*cvQueryFrame2(capture, sl_params)*/;
	...
	for(int i=0; i<(n_cols+n_rows+1); i++){
		...
		cvCopyImage(cam_frame, cam_codes[2*i]);			//cvCopy的简单宏定义

		// Display inverse code.
		//显示反码
		cvSubRS(proj_codes[i], cvScalar(255), inverse_code);		//255 - proj = inverse
		cvCopy(inverse_code, proj_frame);		//反码复制，inverse -> proj_code
		cvScale(proj_frame, proj_frame, 2.*(sl_params->proj_gain/100.), 0);
		cvShowImage("projWindow", proj_frame);
		cvWaitKey(sl_params->delay);
		//cam_frame = cvQueryFrameSafe(capture, sl_params, false);
...
int displayDecodingResults(IplImage*& decoded_cols, 
	...
	if(sl_params->scan_cols){
		...
		cvSaveImage("color_decoded_cols.bmp", temp_2);
	...
	if(sl_params->scan_rows){
		...
		colorizeWinter(temp_1, temp_2, mask);
		cvShowImageResampled("camWindow", temp_2, sl_params->window_w, sl_params->window_h);
		cvSaveImage("color_decoded_rows.bmp", temp_2);
...
int runStructuredLight(/*CvCapture* capture,*/ 
	...
	char str[128];
	for (int i = 0; i < (gray_ncols + gray_nrows); i++)
	{
		sprintf( str, "%02d.bmp", 2*i);
		cvSaveImage( str, cam_gray_codes[2*i]);
		sprintf( str, "%02d.bmp", 2*i+1);
		cvSaveImage( str, cam_gray_codes[2*i+1]);
	}
	printf("Image save.\n");
	...
	// 保存图片
	cvSaveImage("gray_decoded_cols.bmp", gray_decoded_cols);
	cvSaveImage("gray_decoded_rows.bmp", gray_decoded_rows);
	cvSaveImage("gray_mask.bmp", gray_mask);


	// 显示对应结果
	//cvShowImage("decode", gray_decoded_cols);
	//cvWaitKey(-1);
	//cvShowImage("decode", gray_decoded_rows);
	//cvWaitKey(-1);
